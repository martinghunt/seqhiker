...
func _setup_go_panel() -> void:
	...
	if _go_genome_option != null:
		_go_genome_option.item_selected.connect(func(_index: int) -> void:
			if _app_mode == APP_MODE_COMPARISON:
				_refresh_go_comparison_contigs()
		)
...
func _refresh_go_chromosomes() -> void:
	...
	if _app_mode == APP_MODE_COMPARISON:
		...
		var comparison_genomes: Array[Dictionary] = _comparison_controller.get_genomes() if _comparison_controller != null else []
		for genome_any in comparison_genomes:
...
func _refresh_go_comparison_contigs() -> void:
	if _go_chr_option == null:
		return
	_go_chr_option.clear()
	var selected_genome_id := int(_go_genome_option.get_selected_id()) if _go_genome_option != null and _go_genome_option.item_count > 0 else -1
...
func _apply_go_request() -> void:
	...
	if _app_mode == APP_MODE_COMPARISON:
		if _comparison_controller == null or not _comparison_controller.has_genomes():
			_set_go_status("No comparison genomes loaded.", true)
			return
		var selected_genome_id := int(_go_genome_option.get_selected_id()) if _go_genome_option != null else -1
		var genome_len := 0
		var genome_name := ""
		var segment: Dictionary = {}
		for genome_any in _comparison_controller.get_genomes():
			var genome: Dictionary = genome_any
			if int(genome.get("id", -1)) == selected_genome_id:
				...
				if seg_idx >= 0:
					...
					if typeof(seg_meta) == TYPE_DICTIONARY:
						segment = seg_meta
				break
		if genome_len <= 0 or segment.is_empty():
			_set_go_status("Genome length unavailable.", true)
			return
		...
		var cmp_start_display := _parse_go_bp(_go_start_edit.text)
		if cmp_start_display < 1:
			_set_go_status("Enter a valid start position.", true)
			return
		var cmp_end_display := _parse_go_bp(_go_end_edit.text) if _go_end_edit != null else -1
		if cmp_start_display > seg_len:
			_set_go_status("Start position beyond chromosome length.", true)
			return
		if cmp_end_display >= 0 and cmp_end_display < cmp_start_display:
			var cmp_swap := cmp_start_display
			cmp_start_display = cmp_end_display
			cmp_end_display = cmp_swap
		if cmp_end_display > seg_len:
			cmp_end_display = seg_len
		var cmp_start_bp := seg_start + cmp_start_display - 1
		var cmp_end_bp := seg_start + cmp_end_display if cmp_end_display >= 0 else cmp_start_bp + 1
		if cmp_end_display >= 0:
			_comparison_controller.focus_genome_range_with_zoom(selected_genome_id, cmp_start_bp, cmp_end_bp)
			_set_go_status("%s / %s:%d-%d" % [genome_name, seg_name, cmp_start_display, cmp_end_display])
		else:
			_comparison_controller.focus_genome_range(selected_genome_id, cmp_start_bp, cmp_end_bp)
			_set_go_status("%s / %s:%d" % [genome_name, seg_name, cmp_start_display])
		_close_feature_panel()
		return
	...
	if _seq_view_mode == SEQ_VIEW_CONCAT:
		for seg in _concat_segments:
			if int(seg.get("id", -1)) == selected_chr_id:
